...
export default () => ({
  ...
  initialize() {
    ...
    this.maxSlideIndex = slides.reduce((acc, cur, i) => {
      if (cur.clientHeight > acc.clientHeight) {
        return i;
      } else {
        return acc;
      }
  ...
  adjustContainerHeight() {
    ...
    const maxSlideHeight =
      this.$refs[`slide-${this.maxSlideIndex}`].clientHeight;
  ...
  previousSlide() {
    this.resetInterval();
    let index =
      ((this.currentIndex - 2 + this.totalSlides) % this.totalSlides) + 1;
